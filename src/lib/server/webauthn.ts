...
export function getUserPasskeyCredentials(
  userId: number
...
export function deleteUserPasskeyCredential(
  userId: number,
  credentialId: Uint8Array
): boolean {
  const result = db.execute(
    "DELETE FROM passkey_credential WHERE id = ? AND user_id = ?",
    [credentialId, userId]
  )
  return result.changes > 0
}

export function getUserSecurityKeyCredentials(
  userId: number
): WebAuthnUserCredential[] {
  const rows = db.query(
...
export function getUserSecurityKeyCredential(
  userId: number,
  credentialId: Uint8Array
): WebAuthnUserCredential | null {
  const row = db.queryOne(
...
export function deleteUserSecurityKeyCredential(
  userId: number,
  credentialId: Uint8Array
): boolean {
  const result = db.execute(
    "DELETE FROM security_key_credential WHERE id = ? AND user_id = ?",
    [credentialId, userId]
  )
  return result.changes > 0
}
